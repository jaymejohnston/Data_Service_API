...
class ApiConfig:
    ...
    def __post_init__(self):
        ...
        if not _ORGANIZATION_ID_PATTERN.match(self.organization_id):
            raise ValueError(
                f"organization_id={self.organization_id!r} is invalid — it must contain only "
                "letters, numbers, underscores, and hyphens (e.g. 'samplecollp'). Did you paste "
                "the Organizations API URL, or the 'id' (UUID) field, instead of the 'customerID' "
                "value from its response? Consider using ApiConfig.from_pak(pak_token) instead, "
    ...
    def from_pak(
        ...
        *,
        ...
        """Build an ApiConfig by resolving the organization directly from the PAK.

        Calls the Organizations API with the PAK to discover which organization(s) it
        can access, instead of requiring organization_id to be configured by hand
        (which is easy to get wrong - see _ORGANIZATION_ID_PATTERN above).

        - If the PAK maps to exactly one organization, it's used automatically.
        - If it maps to more than one (MSP / parent-company PAKs), you must either pass
          organization_id explicitly, or pass on_multiple_organizations - a callback that
          receives the list of OrganizationInfo and returns the one to use (for example,
